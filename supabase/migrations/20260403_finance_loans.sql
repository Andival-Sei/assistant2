alter table public.finance_accounts
    add column if not exists loan_principal_minor bigint null,
    add column if not exists loan_interest_percent numeric(7,3) null,
    add column if not exists loan_payment_amount_minor bigint null,
    add column if not exists loan_payment_due_date date null,
    add column if not exists loan_remaining_payments_count integer null;
...
alter table public.finance_accounts
    ...
    check (kind in ('bank_card', 'cash', 'loan'));
...
begin
    if not exists (
        select 1
        from pg_constraint
        where conname = 'finance_accounts_loan_principal_minor_check'
    ) then
        alter table public.finance_accounts
            add constraint finance_accounts_loan_principal_minor_check
            check (loan_principal_minor is null or loan_principal_minor > 0);
    end if;

    if not exists (
        select 1
        from pg_constraint
        where conname = 'finance_accounts_loan_interest_percent_check'
    ) then
        alter table public.finance_accounts
            add constraint finance_accounts_loan_interest_percent_check
    ...
    if not exists (
        select 1
        from pg_constraint
        where conname = 'finance_accounts_loan_payment_amount_minor_check'
    ) then
        alter table public.finance_accounts
            add constraint finance_accounts_loan_payment_amount_minor_check
            check (loan_payment_amount_minor is null or loan_payment_amount_minor > 0);
    end if;

    if not exists (
        select 1
        from pg_constraint
        where conname = 'finance_accounts_loan_remaining_payments_count_check'
    ) then
        alter table public.finance_accounts
            add constraint finance_accounts_loan_remaining_payments_count_check
            check (loan_remaining_payments_count is null or loan_remaining_payments_count >= 0);
    end if;
end $$;

alter table public.finance_accounts
    drop constraint if exists finance_accounts_credit_fields_consistency_check;
...
alter table public.finance_accounts
    drop constraint if exists finance_accounts_loan_fields_consistency_check;

alter table public.finance_accounts
    add constraint finance_accounts_loan_fields_consistency_check
    check (
        (
            kind = 'loan'
        ...
        or (
            loan_principal_minor is null
            and loan_interest_percent is null
            and loan_payment_amount_minor is null
            and loan_payment_due_date is null
            and loan_remaining_payments_count is null
        )
    );

update public.finance_accounts
set loan_principal_minor = null,
    loan_interest_percent = null,
    loan_payment_amount_minor = null,
    loan_payment_due_date = null,
    loan_remaining_payments_count = null
where kind <> 'loan';

update public.finance_user_settings
set overview_cards = overview_cards || '["loan_debt"]'::jsonb
where not coalesce(overview_cards, '[]'::jsonb) @> '["loan_debt"]'::jsonb;
...
begin
    ...
    if v_account.kind = 'bank_card' and v_account.card_type = 'credit' then
        ...
        v_next_debt := case
            when p_direction = 'income' then greatest(balance_minor - p_amount_minor, 0)
...
drop function if exists public.finance_upsert_account(uuid, text, text, text, bigint, text, boolean, bigint, bigint, bigint, date, date);
...
begin
    ...
    if p_id is null then
        ...
        insert into public.finance_accounts (
            ...
            loan_payment_amount_minor,
            loan_payment_due_date,
            loan_remaining_payments_count,
            is_primary,
            display_order
        )
        values (
            ...
            v_loan_principal_minor,
            v_loan_interest_percent,
            v_loan_payment_amount_minor,
            case when v_kind = 'loan' then p_loan_payment_due_date else null end,
            v_loan_remaining_payments_count,
